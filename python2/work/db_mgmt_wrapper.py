...
class Postgres(Base):
    ...
    def create_user(self):
        ...
        try:
            ...
            ans = raw_input("Do you want to assign privileges as well to "
                ...
                if resp == 'rw':
                    if db_name not in ('postgres', 'template0', 'template1'):
                        ...
                        c.execute(
                            ...
                            (self.priv_mode, self.adjective, self.u_name))
                        ans = raw_input("Since this is a RW user, do you "
...
def main():
    ...
    def show_mysql_running_instances():
        ports = sb.Popen(
            "ps -ef | grep mysql | grep -E -o 'port=+[0-9]+' |"
            "sort -u",
            stdout=sb.PIPE,
            stderr=sb.PIPE,
